...
def calipso_sort(filename,hmin,hmax,hemi):
    import warnings
    ...
    import numpy as np
    if hemi=='s':
        lat,lon,Image2,Image2_qa=c.file_sort_s()
        lat=np.repeat(lat,5)
        lon=np.repeat(lon,5)
    if hemi=='n':
        lat,lon,Image2,Image2_qa=c.file_sort_n()
        lat=np.repeat(lat,5)
        lon=np.repeat(lon,5)
    #c=np.where((lat<-30)&(lat>-68))#new change
    #lat=lat[c]
    #lon=lon[c]
    #Image2=Image2[:,c[0]]
    #Image2_qa=Image2_qa[:,c[0]]
    height=np.array((range(-500,8200,30)+range(8200,20200,60)+range(20200,30100,180))*Image2.shape[1]).reshape(Image2.shape[1],545)
    #Image2_qa=np.nansum(Image2_qa,axis=0)
    c=np.where((Image2_qa<6) & (Image2_qa>0))
    Image2[c]=6
    #lets create a cloud top height
    ...
    height1=np.zeros(p2.shape)
    height1[c4]=height.T[c4]
    height1_min=np.zeros(p2.shape)
    height1_min[c5]=height.T[c5]

    height_array=np.max(height1,axis=0)
    ...
    std=np.array([np.nanstd(height_array[i:i+30]) for i in range(len(height_array))])
    ctp2=cloud_top_phase
    ctp3=cloud_top_phase#cloud top_phase for different height intervals
    c=np.where((height_array<hmin)&(height_array>hmax)&(diff_thick>2000)&((std<1000)&(std>20)))
    cloud_top_phase[c]=np.nan

    c=np.where((height_array<3500)&(height_array>7000)&(std<1000))#condition for higher clouds
    ctp2[c]=np.nan
    c=np.where((height_array<hmin)&(height_array>hmax))
    ctp3[c]=np.nan
    #TODO 
    ...
    return cloud_top_phase,ctp2,ctp3,height_array
...
class Cal2(_Cal):
    ...
    def file_sort_n(self,idx=None):
        ...
        c_uce=np.where(((lat>40) &(lat<64))&((lon>-55) &(lon<-10)))
